...
(function() {
  function wordcloud() {
    ...
    function draw(data, bounds) {
      ...
      text.style("font-family", function(d) {
        ...
        .on("mouseover", function(d, i) {
          if (d.href) {
            d3.select(this).transition().style('font-size', d.size + 1 + 'px');
          }
        })
    ...
    function update() {
      ...
      fontSize = d3.scale[scale]().range([5, 80]);
      if (words.length) {
        fontSize.domain([words[words.length - 1].size-5 || 1, +words[0].size+5]);
      }
    }
